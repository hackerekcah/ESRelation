...
@register_dataset
class UrbanSound8KME(torch.utils.data.Dataset):
    """
    ...
    def __init__(self, fold, split, target_sr=44100, transform=None):
        super(UrbanSound8KME, self).__init__()
        if target_sr == 44100:
    ...
    def _multi_path(self, file):
        path_splits = file.split('/')
        file_list = []
        semitones = [-3.5, 2.5, 2.5, 3.5]
        # semitones = [2.5, 2.5]

        file_list.append(file)
        for s in semitones:
            ps_path_splits = copy.deepcopy(path_splits)
            ps_path_splits[-3] = "{}-ps{:.1f}".format(path_splits[-3], s)
            ps_path = '/'.join(ps_path_splits)
            file_list.append(ps_path)
    ...
    def __getitem__(self, idx):
        ...
        file = self.files[idx]
        multi_audio = []
        for file in self._multi_path(file):
            with torch.no_grad():
                # (channels, frames)
                ...
                multi_audio.append(audio)

        audio = torch.cat(multi_audio)
...
if __name__ == '__main__':
    dataset = UrbanSound8KME(fold=2, split='valid', target_sr=22050)
